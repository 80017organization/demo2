...
var Service = function () {
  ...
  _createClass(Service, [{
    ...
    key: 'watchChangefeeds',
    value: function watchChangefeeds(app) {
      var _this3 = this;
      ...
      var runHooks = function runHooks(method, data) {
      ...
      if (this.__hooks) {
        ...
        runHooks = function runHooks(method, data) {
          var service = _this3;
          var args = [{ query: {} }];
          var hookData = {
            app: app,
          ...
          if (method === 'update' || method === 'patch' || method === 'remove') {
            args.unshift(data[_this3.id]);
          }

          var hookObject = _feathersCommons.hooks.hookObject(method, 'after', args, hookData);
          var hookChain = (0, _commons.getHooks)(app, _this3, 'after', method);

          return _commons.processHooks.call(_this3, hookChain, hookObject);
        };
      }
      ...
      this._cursor = this.table.changes().run().then(function (cursor) {
        cursor.each(function (error, data) {
          ...
          if (data.old_val === null) {
            runHooks('create', data.new_val).then(function (hook) {
              return _this3.emit('created', hook.result);
            });
          } else if (data.new_val === null) {
            runHooks('remove', data.old_val).then(function (hook) {
              return _this3.emit('removed', hook.result);
            });
          } else {
            runHooks('patch', data.new_val).then(function (hook) {
              _this3.emit('updated', hook.result);
              _this3.emit('patched', hook.result);
            });
          }
        });
